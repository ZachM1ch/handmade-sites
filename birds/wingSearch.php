	...
	?>
		...
		<tr>
			<td>
				...
				?>
				<input type = "submit" value = "Switch to Alphabetical">
			</td>
			<td>
				<?php
				echo '<h2 class="p2"><u>Search For "'.$searchTerm.'" (wingspan)</u></h2>';
			?>
			</td>
			<td>
		...
		<?php 
			...
			$query = "select id, colloquial_name, image_path, avg_wingspan from birds where colloquial_name like '%".$searchTerm."%'";
			...
			//Sorting contents of $rows by wingspan
			for($i = 0; $i < $count; $i++)
			{
				for ($j = 0; $j < $count - $i - 1; $j++)
				{
					if ($rows[$j][3] > $rows[$j+1][3])
					{
						$temp = $rows[$j];
						$rows[$j] = $rows[$j+1];
						$rows[$j+1] = $temp;
			...
			for($i = 0; $i < ceil($count/4); $i++)
			{
				...
				for($j = 0; $j < 4; $j++)
				{
					...
					{
						echo "<td>";
						$titleString = "<p class='p3'>".$rows[$birdCounter][1]." (".$rows[$birdCounter][3]."cm)</p>";
						echo $titleString;
